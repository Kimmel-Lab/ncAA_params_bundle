...
def order(x, NoneIsLast = True, decreasing = False):
    ...
    ix = list(range(n))
    if None not in x:
        ix = sorted(range(n), key=lambda j: x[j], reverse=decreasing)
    else:
        def key(i, x=x):
            elem = x[i]
            if decreasing == NoneIsLast:
                return (not(elem is None), elem if elem is not None else 0)
            else:
                return (elem is None, elem if elem is not None else 0)
        ix = sorted(range(n), key=key, reverse=decreasing)
